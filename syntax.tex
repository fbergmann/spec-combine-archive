...
\begin{itemize}
	\item  {
     ...
     \begin{itemize}
		...
		\item \token{Bag} (\url{http://www.w3.org/1999/02/22-rdf-syntax-ns#Bag},
		\item \token{li} (\url{http://www.w3.org/TR/2004/REC-rdf-syntax-grammar-20040210/#section-Syntax-list-elements}),
	\end{itemize}
	}
	\item  {
	...
	\token{dcterms:modified} elements{\footnote{\url{http://www.w3.org/TR/NOTE-datetime}}}.
	}
...
in the SBML vCard specification and to follow W3C recommendations {\footnote[4]{\url{http://www.w3.org/TR/vcard-rdf/}}} . The changes are: 
...
\begin{itemize}
	...
	\item "rdf:Bags" anf "rdf:li" are no longer mandatory if there is only one statement.
\end{itemize}

A \textit{COMBINE archive} can include multiple metadata elements adding information about different content files. To 
identify the file a metadata element refers too, the \token{rdf:about} attribute of the relevant metadata structure should use the same value as used in the \token{location} attribute of the respective \Content element, e.g.
